...
<head>
  ...
  <title>Veterinary Clinic Management System | Veterinary Account</title>
...
<div class="card mb-4 mx-3 mt-4">
    <div class="card-body">
        <div class="section-title">
            <h4 class="m-0 text-uppercase font-weight-bold">Veterinary Account List</h4>
        </div>
        <br>
...
try {
    ...
    foreach ($rows as $index => $row) {
        echo "<tr>";
        echo "<td>" . ($index + 1) . "</td>";
        $imageSrc = !empty($row['vet_image']) ? htmlspecialchars($row['vet_image']) : 'https://www.w3schools.com/w3images/avatar5.png';
        ...
        $stats = htmlspecialchars($row['stats']);
        $badgeClass = '';
        $statusText = '';
...
echo "<a href='edit_veterinary_account.php?id=" . urlencode($row['id']) . 
     "&vet_image=" . urlencode($row['vet_image']) . 
     "&clinic_name=" . urlencode($row['clinic_name']) . 
     "&fullname=" . urlencode($row['fullname']) . 
     "&email=" . urlencode($row['email']) . 
     "&password=" . urlencode($row['password']) . 
     "&stats=" . urlencode($row['stats']) . 
     "' class='btn btn-warning btn-sm'> <i class='fa fa-edit'></i></a>";
echo "<button class='btn btn-danger btn-sm delete-btn' data-id='" . htmlspecialchars($row['id']) . "'><i class='fa fa-trash'></i></button>";
        echo "</td>";
        echo "</tr>";
    }
} catch (PDOException $e) {
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.delete-btn').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure you want to delete this veterinary account?',
                text: "",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('delete_veterinary_account.php?id=' + encodeURIComponent(id))
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                showToast('Veterinary account deleted successfully.','success');
