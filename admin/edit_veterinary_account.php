...
if (isset($_GET['id']) && isset($_GET['vet_image']) && isset($_GET['clinic_name']) && isset($_GET['fullname']) && isset($_GET['email']) && isset($_GET['password']) && isset($_GET['stats'])) {
    // Extract values from URL parameters
    $id = urldecode($_GET['id']);
    $vet_image = urldecode($_GET['vet_image']);
    $clinic_name = urldecode($_GET['clinic_name']);
    $fullname = urldecode($_GET['fullname']);
    $email = urldecode($_GET['email']);
    $password = urldecode($_GET['password']);
    $stats = urldecode($_GET['stats']);
} else {
...
<form id="updatevetForm" enctype="multipart/form-data">
  <input type="hidden" name="id" value="<?php echo htmlspecialchars($id); ?>">
    ...
    <div class="form-group">
        ...
        <input type="email" class="form-control" id="email" name="email" placeholder="Enter email" value="<?php echo htmlspecialchars($email); ?>" readonly>
    </div>
    <div class="form-group">
        <label for="password">Password</label>
        <input type="password" class="form-control" id="password" name="password" placeholder="Enter password" value="<?php echo htmlspecialchars($password); ?>" readonly>
    </div>
    <div class="form-group">
...
<div class="form-group">
    ...
    <select id="stats" name="stats" class="form-control">
        <option value="active" <?php echo ($stats === 'active') ? 'selected' : ''; ?>>Active</option>
        <option value="inactive" <?php echo ($stats === 'inactive') ? 'selected' : ''; ?>>Inactive</option>
...
<div class="form-group position-relative">
    <img id="vet-img" src="<?php echo htmlspecialchars($vet_image ?? 'https://www.w3schools.com/w3images/avatar5.png'); ?>" alt="Clinic Picture" class="img-fluid">
    <input type="file" name="vet_image" id="vet-upload" accept="image/*" style="display: none;">
    <label for="vet-upload" class="btn btn-primary position-absolute bottom-0 start-50 translate-middle-x">Choose Clinic Picture</label>
</div>

    </div>
    <button type="submit" class="btn btn-primary">Submit</button>
    <a href="veterinary_account.php" class="btn btn-primary">Back</a>
</form>
                </div>
            </div>
...
$(document).ready(function() {
    $('#updatevetForm').on('submit', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    setTimeout(function() {
                        window.location.href = 'veterinary_account.php'; // Modify this if you need to pass parameters
                    }, 3000);
                } else {
