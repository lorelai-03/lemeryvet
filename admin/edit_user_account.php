...
if (isset($_GET['id']) && isset($_GET['user_image']) && isset($_GET['full_name']) && isset($_GET['email']) && isset($_GET['password']) && isset($_GET['status'])) {
    // Extract values from URL parameters
    $id = urldecode($_GET['id']);
    $user_image = urldecode($_GET['user_image']);
...
<form id="updateuserForm" enctype="multipart/form-data">
  <input type="hidden" name="id" value="<?php echo htmlspecialchars($id); ?>">
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="full_name" name="full_name" placeholder="Enter full name" value="<?php echo htmlspecialchars($full_name); ?>">
    </div>
    <div class="form-group">
...
<div class="form-group">
    ...
    <select id="status" name="status" class="form-control">
        ...
        <option value="inactive" <?php echo ($status === 'inactive') ? 'selected' : ''; ?>>Block</option>
...
<div class="form-group position-relative">
    <img id="user-img" src="<?php echo htmlspecialchars($user_image ?? 'https://www.w3schools.com/w3images/avatar5.png'); ?>" alt="Clinic Picture" class="img-fluid">
    <input type="file" name="user_image" id="user-upload" accept="image/*" style="display: none;">
    <label for="user-upload" class="btn btn-primary position-absolute bottom-0 start-50 translate-middle-x">Choose Clinic Picture</label>
</div>

    </div>
    <button type="submit" class="btn btn-primary">Submit</button>
    <a href="customer_account.php" class="btn btn-primary">Back</a>
</form>
                </div>
            </div>
...
$(document).ready(function() {
    $('#updateuserForm').on('submit', function(event) {
        ...
        $.ajax({
            url: 'update_user_account.php', // PHP processing file
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            ...
            success: function(response) {
                ...
                if (response.success) {
                    showToast('User account update successfully.','success');
