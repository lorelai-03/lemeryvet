...
<head>
  ...
  <title>Veterinary Clinic Management System | Clinic Information</title>
...
try {
    ...
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $pdo->query("SELECT * FROM clinic_info");
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($rows as $index => $row) {
        echo "<tr>";
        echo "<td>" . ($index + 1) . "</td>";
        echo "<td><img src='" . htmlspecialchars($row['clinic_image']) . "' alt='Clinic Image' width='50'></td>";
        echo "<td>" . htmlspecialchars($row['business_permit_no']) . "</td>";
        echo "<td>" . htmlspecialchars($row['clinic_name']) . "</td>";
        echo "<td>" . htmlspecialchars($row['address']) . "</td>";
        echo "<td>" . htmlspecialchars($row['line_of_business']) . "</td>";
...
echo "<a href='edit_clinic_information.php?id=" . urlencode($row['id']) . 
     "&clinic_image=" . urlencode($row['clinic_image']) . 
     "&business_permit_no=" . urlencode($row['business_permit_no']) . 
     "&clinic_name=" . urlencode($row['clinic_name']) . 
     "&address=" . urlencode($row['address']) . 
     "&line_of_business=" . urlencode($row['line_of_business']) . 
        ...
        button.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('delete_clinic_information.php?id=' + encodeURIComponent(id))
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                showToast('Clinic information deleted successfully.','success');
