...
$pet_name = isset($pet_name) ? $conn->real_escape_string($pet_name) : '';
...
if (empty($id)) {
    // Insert new record
    $sql = "INSERT INTO `schedule_list` (`title`, `description`, `start_datetime`, `end_datetime`, `veterinaryId`, `medicalName`, `full_name`, `pet_name`) 
            VALUES ('$title', '$description', '$start_datetime', '$end_datetime', '$veterinaryId', '$medicalName', '$full_name', '$pet_name')";
    $action = 'saved';
} else {
    ...
    $sql = "UPDATE `schedule_list` 
            SET `title` = '{$title}', `description` = '{$description}', `start_datetime` = '{$start_datetime}', 
                ...
                `full_name` = '{$full_name}', `pet_name` = '{$pet_name}'
            WHERE `id` = '{$id}'";
    $action = 'updated';
}
...
if ($save) {
    ...
    try {
        ...
        $mail->Body    = 'Dear ' . htmlspecialchars($full_name) . ',<br><br>' .
                         ...
                         'End: ' . $end_datetime_formatted . '<br>' .
                         'Pet Name: ' . htmlspecialchars($pet_name) . '<br><br>' .
