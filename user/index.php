...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("SELECT id, full_name, password, status, verified, phone_verified FROM users WHERE email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->store_result();
    
    if ($stmt->num_rows > 0) {
        $stmt->bind_result($id, $fullname, $hashed_password, $status, $verified, $phone_verified);
        $stmt->fetch();
        ...
        if ($status === 'inactive') {
            ...
            if ($verified == 0) {
                ...
                exit();
            } elseif ($phone_verified == 0) {
                header("Location: phone_verification.php");
                exit();
            } else {
                // Set session variables and redirect to home if both verifications are done
                $_SESSION['id'] = $id; // Store the user ID in session
                $_SESSION['email'] = $email; // Optionally store email in session
                $_SESSION['full_name'] = $fullname; // Store the user's full name in session
...
<div class="login-box">
  ...
  <div class="card">
    ...
    <a href="../../index2.html" style="color: black;"><b></b>USER LOGIN</a>
  </div>
