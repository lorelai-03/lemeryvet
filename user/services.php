...
$db_user = 'u104053626_lemeryvetcare'; // Database username
$db_password = 'Lemeryvet4209*'; // Database password
...
// Get the veterinary ID from the GET request
$veterinaryId = isset($_GET['id']) ? intval($_GET['id']) : null;

// Check if the user is logged in and the veterinary ID is valid
if (isset($_SESSION['email']) && $veterinaryId !== null) {
    // Fetch the latest record from medical_product_percentage for the given veterinary ID
    $query = "SELECT percentage FROM medical_product_percentage WHERE veterinary_id = ? ORDER BY id DESC LIMIT 1";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $veterinaryId); // Bind the veterinary ID to the query
    $stmt->execute();
    $result = $stmt->get_result();

    // Get the latest percentage
    $latest_percentage = $result->num_rows > 0 ? $result->fetch_assoc()['percentage'] : 0; // Default to 0 if no records
    
    $stmt->close();
    $conn->close();
} else {
    $latest_percentage = null; // Invalid session or ID
}

// Prepare the output variable based on the latest percentage value
if ($latest_percentage === 0) {
    $output = '<h1 class="text-white mb-3" style="font-weight: 1000;">Medical Products</h1>';
} elseif ($latest_percentage !== null) {
    $output = '<h1 class="text-white mb-3" style="font-weight: 1000;">Medical Products Up To <span style="color: #ffc107;">' . $latest_percentage . '%</span> Off</h1>';
} else {
    $output = '<h1 class="text-white mb-3" style="font-weight: 1000;">You are not logged in or the veterinary ID is invalid.</h1>';
}
?>
...
<section class="py-5" id="medical">
    <div class="container">
        ...
        // Capture the veterinarian's ID from the URL (i.e., the clicked button)
        $veterinaryId = isset($_GET['id']) ? $_GET['id'] : '';

        // Optionally capture the 'clinic_name' parameter as well
        $clinicName = isset($_GET['clinic_name']) ? urldecode($_GET['clinic_name']) : '';

        // Check if a search query is set
        $searchQuery = isset($_GET['search']) ? $_GET['search'] : '';

        // Prepare the SQL query with a WHERE clause for filtering by veterinary_id and search query
        $sql = "SELECT id, veterinary_id, medical_name, medical_description, medical_price, medical_image 
                FROM medical_products 
                WHERE veterinary_id = :veterinaryId"; // Filter by the clicked veterinarian's id

        // If a search query is provided, add an additional condition to the query
        if ($searchQuery) {
            $sql .= " AND medical_name LIKE :searchQuery"; // Combine veterinary_id and search filter
        }

        $stmt = $pdo->prepare($sql);

        // Bind the veterinarian's id to the query
        $stmt->bindValue(':veterinaryId', $veterinaryId, PDO::PARAM_INT);

        // If a search query is provided, bind the search term as well
        if ($searchQuery) {
        ...
        // Fetch the filtered medical products
        $medicalProducts = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ?>

        <!-- Display Cards -->
        <div class="container">
            <div class="row">
                <?php if ($medicalProducts): ?>
                    <?php foreach ($medicalProducts as $product): ?>
                        <div class="col-md-4 mb-4"> <!-- Each card takes 4 columns, making it 3 cards per row -->
                            <div class="card" style="width: 100%; height: 26rem;">
                                <?php if (!empty($product['medical_image'])): ?>
                                    <img class="card-img-top" src="<?php echo htmlspecialchars($product['medical_image']); ?>" alt="<?php echo htmlspecialchars($product['medical_name']); ?>" style="object-fit: cover; height: 10rem;">
                                <?php else: ?>
                                    <img class="card-img-top" src="path/to/default/image.jpg" alt="Default Image" style="object-fit: cover; height: 10rem;">
                                <?php endif; ?>
                                <div class="card-body d-flex flex-column">
                                    <h5 class="card-title" style="text-align: center; margin-bottom: 10px; font-weight: bold;">
                                        <?php echo htmlspecialchars($product['medical_name']); ?>
                                    </h5>
                                    <p class="card-text" style="flex-grow: 1; text-align: justify; text-justify: inter-word; max-height: 100px; overflow-y: auto; scrollbar-width: none; -ms-overflow-style: none;">
                                        <?php echo htmlspecialchars($product['medical_description']); ?>
                                    </p>
                                    <div class="d-flex justify-content-between align-items-center mt-auto">
                                        <span class="price" style="font-weight: bold; text-align: right;">₱<?php echo number_format($product['medical_price'], 2); ?></span>
                                        <a href="#" class="btn btn-warning mx-1 appointment-btn" data-id="<?php echo $product['id']; ?>" data-veterinary-id="<?php echo $product['veterinary_id']; ?>" data-medical-name="<?php echo $product['medical_name']; ?>"><i class="fa fa-calendar"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p class="text-center">No medical products found.</p>
                <?php endif; ?>
            </div>
        </div>
...
// Check if the user is logged in and the veterinary ID is valid
if (isset($_SESSION['email']) && $veterinaryId !== null) {
    // Fetch the latest record from veterinary_services_percentage for the given veterinary ID
    $query = "SELECT percentage FROM veterinary_services_percentage WHERE veterinary_id = ? ORDER BY id DESC LIMIT 1";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $veterinaryId); // Bind the veterinary ID to the query
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    // Get the latest percentage
    $veterinary_latest_percentage = $result->num_rows > 0 ? $result->fetch_assoc()['percentage'] : 0; // Default to 0 if no records
    
    $stmt->close();
    $conn->close();
} else {
    $veterinary_latest_percentage = null; // Invalid session or ID
}

// Prepare the output variable based on the latest percentage value
if ($veterinary_latest_percentage === 0) {
    $output_veterinary = '<h1 class="text-white mb-3" style="font-weight: 1000;">Veterinary Services</h1>';
} elseif ($veterinary_latest_percentage !== null) {
    $output_veterinary = '<h1 class="text-white mb-3" style="font-weight: 1000;">Veterinary Services Up To <span style="color: #ffc107;">' . $veterinary_latest_percentage . '%</span> Off</h1>';
} else {
    $output_veterinary = '<h1 class="text-white mb-3" style="font-weight: 1000;">You are not logged in or the veterinary ID is invalid.</h1>';
}
...
<section class="py-5" id="veterinary">
    <div class="container">
        ...
        // Prepare the SQL query with a WHERE clause for filtering by veterinary_id and search query
        $sql = "SELECT id, veterinary_id, veterinary_name, veterinary_description, veterinary_price, veterinary_image 
                FROM veterinary_services 
                WHERE veterinary_id = :veterinaryId"; // Filter by the clicked veterinarian's id

        // If a search query is provided, add an additional condition to the query
        if ($searchQuery) {
            $sql .= " AND veterinary_name LIKE :searchQuery"; // Combine veterinary_id and search filter
        }
        ...
        // Fetch the filtered veterinary services
        $veterinaryServices = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ?>

        <!-- Display Cards -->
        <div class="container">
            <div class="row">
                <?php if ($veterinaryServices): ?>
                    <?php foreach ($veterinaryServices as $service): ?>
                        <div class="col-md-4 mb-4"> <!-- Each card takes 4 columns, making it 3 cards per row -->
                            <div class="card" style="width: 100%; height: 26rem;">
                                <?php if (!empty($service['veterinary_image'])): ?>
                                    <img class="card-img-top" src="<?php echo htmlspecialchars($service['veterinary_image']); ?>" alt="<?php echo htmlspecialchars($service['veterinary_name']); ?>" style="object-fit: cover; height: 10rem;">
                                <?php else: ?>
                                    <img class="card-img-top" src="path/to/default/image.jpg" alt="Default Image" style="object-fit: cover; height: 10rem;">
                                <?php endif; ?>
                                <div class="card-body d-flex flex-column">
                                    <h5 class="card-title" style="text-align: center; margin-bottom: 10px; font-weight: bold;">
                                        <?php echo htmlspecialchars($service['veterinary_name']); ?>
                                    </h5>
                                    <p class="card-text" style="flex-grow: 1; text-align: justify; text-justify: inter-word; max-height: 100px; overflow-y: auto; scrollbar-width: none; -ms-overflow-style: none;">
                                        <?php echo htmlspecialchars($service['veterinary_description']); ?>
                                    </p>
                                    <div class="d-flex justify-content-between align-items-center mt-auto">
                                        <span class="price" style="font-weight: bold; text-align: right;">₱<?php echo number_format($service['veterinary_price'], 2); ?></span>
                                        <a href="#" class="btn btn-warning mx-1 appointment-btn" data-id="<?php echo $service['id']; ?>" data-veterinary-id="<?php echo $service['veterinary_id']; ?>" data-veterinary-name="<?php echo $service['veterinary_name']; ?>"><i class="fa fa-calendar"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p class="text-center">No veterinary services found.</p>
                <?php endif; ?>
            </div>
        </div>
...
// Check if the user is logged in and the veterinary ID is valid
if (isset($_SESSION['email']) && $veterinaryId !== null) {
    // Fetch the latest record from pet_product_percentage for the given veterinary ID
    $query = "SELECT percentage FROM pet_product_percentage WHERE veterinary_id = ? ORDER BY id DESC LIMIT 1";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $veterinaryId); // Bind the veterinary ID to the query
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    // Get the latest percentage
    $pet_latest_percentage = $result->num_rows > 0 ? $result->fetch_assoc()['percentage'] : 0; // Default to 0 if no records
    
    $stmt->close();
    $conn->close();
} else {
    $pet_latest_percentage = null; // Invalid session or ID
}

// Prepare the output variable based on the latest percentage value
if ($pet_latest_percentage === 0) {
    $output_pet = '<h1 class="text-white mb-3" style="font-weight: 1000;">Pet Supplies</h1>';
} elseif ($pet_latest_percentage !== null) {
    $output_pet = '<h1 class="text-white mb-3" style="font-weight: 1000;">Pet Supplies Up To <span style="color: #ffc107;">' . $pet_latest_percentage . '%</span> Off</h1>';
} else {
    $output_pet = '<h1 class="text-white mb-3" style="font-weight: 1000;">You are not logged in or the veterinary ID is invalid.</h1>';
} 
...
<section class="py-5" id="pet">
    <div class="container">
        ...
        // Capture the veterinarian's ID from the URL
        $veterinaryId = isset($_GET['id']) ? $_GET['id'] : '';
        ...
        // Prepare the SQL query with a WHERE clause for filtering by veterinary_id and search query
        $sql = "SELECT id, veterinary_id, pet_name, pet_description, pet_price, pet_image 
                FROM pet_products 
                WHERE veterinary_id = :veterinaryId"; // Filter by the clicked veterinarian's id

        // If a search query is provided, add an additional condition to the query
        if ($searchQuery) {
            $sql .= " AND pet_name LIKE :searchQuery"; // Combine veterinary_id and search filter
        }

        $stmt = $pdo->prepare($sql);

        // Bind the veterinary_id to the query
        $stmt->bindValue(':veterinaryId', $veterinaryId, PDO::PARAM_INT);
        ...
        // Fetch the filtered pet supplies
        $petSupplies = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ?>
        ...
        <div class="container">
            <div class="row">
                <?php if ($petSupplies): ?>
                    <?php foreach ($petSupplies as $supply): ?>
                        <div class="col-md-4 mb-4"> <!-- Each card takes 4 columns, making it 3 cards per row -->
                            <div class="card" style="width: 100%; height: 26rem;">
                                <?php if (!empty($supply['pet_image'])): ?>
                                    <img class="card-img-top" src="<?php echo htmlspecialchars($supply['pet_image']); ?>" alt="<?php echo htmlspecialchars($supply['pet_name']); ?>" style="object-fit: cover; height: 10rem;">
                                <?php else: ?>
                                    <img class="card-img-top" src="path/to/default/image.jpg" alt="Default Image" style="object-fit: cover; height: 10rem;">
                                <?php endif; ?>
                                <div class="card-body d-flex flex-column">
                                    <h5 class="card-title" style="text-align: center; margin-bottom: 10px; font-weight: bold;">
                                        <?php echo htmlspecialchars($supply['pet_name']); ?>
                                    </h5>
                                    <p class="card-text" style="flex-grow: 1; text-align: justify; text-justify: inter-word; max-height: 100px; overflow-y: auto; scrollbar-width: none; -ms-overflow-style: none;">
                                        <?php echo htmlspecialchars($supply['pet_description']); ?>
                                    </p>
                                    <div class="d-flex justify-content-between align-items-center mt-auto">
                                        <span class="price" style="font-weight: bold; text-align: right;">₱<?php echo number_format($supply['pet_price'], 2); ?></span>
                                        <a href="#" class="btn btn-warning mx-1 appointment-btn" data-id="<?php echo $supply['id']; ?>" data-veterinary-id="<?php echo $supply['veterinary_id']; ?>" data-pet-name="<?php echo $supply['pet_name']; ?>"><i class="fa fa-calendar"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p class="text-center">No pet supplies found.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
<form id="schedule-form">
    ...
    <div class="modal-body">
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" class="form-control" id="pet_name" name="pet_name" placeholder="Enter your pet's name">
        </div>
        <div class="form-group mb-3">
        ...
        <div class="form-group mb-3">
            <label for="appointmentDescription" class="fw-bold">Pet's Condition</label>
...
$(document).ready(function() {
  $('.appointment-btn').on('click', function() {
    ...
    // Determine the type of button and set the appropriate value for medicalName
    var medicalName;
    if ($(this).data('pet-name') !== undefined) {
      medicalName = $(this).data('pet-name'); // Pet supplies
    } else if ($(this).data('veterinary-name') !== undefined) {
      medicalName = $(this).data('veterinary-name'); // Veterinary services
    } else if ($(this).data('medical-name') !== undefined) {
      medicalName = $(this).data('medical-name'); // Medical products
    }

    // Log the values to the console
    console.log('Veterinary ID:', veterinaryId);
    console.log('Item ID:', itemId);
    console.log('Medical Name:', medicalName);

    // Set the values in the input fields
    $('#veterinaryId').val(veterinaryId);
    $('#medicalName').val(medicalName); // Fill in medicalName based on the button type
