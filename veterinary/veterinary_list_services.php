...
<head>
  ...
  <title>Veterinary Clinic Management System | Veterinary List Services</title>
...
<div class="card mb-4 mx-3 mt-4">
    <div class="card-body">
        <div class="section-title">
            <h4 class="m-0 text-uppercase font-weight-bold">List Veterinary Services</h4>
        </div>
        <br>
<form id="veterinaryServicesForm" enctype="multipart/form-data">
    <div class="form-group">
        <label for="veterinary_name">Veterinary Services Name</label>
        <input type="text" class="form-control" id="veterinary_name" name="veterinary_name" placeholder="Enter veterinary services name" value="">
    </div>
    <div class="form-group">
        <label for="veterinary_description">Veterinary Services Description</label>
        <textarea class="form-control" id="veterinary_description" name="veterinary_description" placeholder="Enter veterinarty services description" rows="3"></textarea>
    </div>
    <div class="form-group">
        <label for="veterinary_price">Veterinary Services Price</label>
        <input type="text" class="form-control" id="veterinary_price" name="veterinary_price" placeholder="Enter veterinary services price" value="">
    </div>
    <div class="form-group">
<div class="form-group position-relative">
    <img id="veterinary-img" src="https://via.placeholder.com/400x300.png?text=Veterinary+List+Image
" alt="Veterinary Picture" class="img-fluid">
    <input type="file" name="veterinary_image" id="veterinary-upload" accept="image/*" style="display: none;">
    <label for="veterinary-upload" class="btn btn-primary position-absolute bottom-0 start-50 translate-middle-x">Choose Image</label>
</div>
...
<script>
    document.getElementById('veterinary-upload').addEventListener('change', function(event) {
        const file = event.target.files[0];
        if (file) {
            const reader = new FileReader();
            reader.onload = function(e) {
                document.getElementById('veterinary-img').src = e.target.result;
            }
    ...
    $(document).ready(function() {
        $('#veterinaryServicesForm').on('submit', function(e) {
            ...
            $.ajax({
                url: 'save_veterinary_services.php', // The URL of your PHP file to handle the form data
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
                    ...
                    try {
                        ...
                        if (res.status === 'success') {
                            // Show success toast
                            showToast('Veterinary services saved successfully!', 'success');
        ...
        $('#veterinary-upload').on('change', function() {
            var reader = new FileReader();
            reader.onload = function(e) {
                $('#veterinary-img').attr('src', e.target.result);
            }
