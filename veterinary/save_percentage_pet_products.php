...
if ($is_logged_in) {
    ...
    $discount_percentage = isset($_POST['percentage1']) ? intval($_POST['percentage1']) : 0;

    // Fetch products that belong to the logged-in user (based on veterinary_id)
    $query = "SELECT id, pet_price, original_price FROM pet_products WHERE veterinary_id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $product_id = $row['id'];
            $original_price = $row['pet_price'];

            // Check if the original_price column is empty and set it if needed
            if (empty($row['original_price'])) {
                $update_original_query = "UPDATE pet_products SET original_price = ? WHERE id = ?";
            ...
            if ($discount_percentage > 0) {
                ...
                // Update the pet_price column with the new discounted price for each product
                $update_query = "UPDATE pet_products SET pet_price = ? WHERE id = ?";
                $update_stmt = $conn->prepare($update_query);
                $update_stmt->bind_param("di", $discounted_price, $product_id);
                $update_stmt->execute();
            } else {
                // Reset to original price
                $update_query = "UPDATE pet_products SET pet_price = original_price WHERE id = ?";
                $update_stmt = $conn->prepare($update_query);
        ...
        // Insert the selected discount percentage into pet_product_percentage table
        if ($discount_percentage >= 0) {
            $insert_percentage_query = "INSERT INTO pet_product_percentage (veterinary_id, percentage) VALUES (?, ?)";
