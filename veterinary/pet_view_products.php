...
if ($is_logged_in) {
    ...
    $sql = "SELECT id, pet_name, pet_description, pet_price, pet_image 
            FROM pet_products 
            WHERE veterinary_id = :user_id";  // Assuming pet_products has a veterinary_id column
    if ($searchQuery) {
        $sql .= " AND pet_name LIKE :searchQuery";
    }
...
<head>
  ...
  <title>Veterinary Clinic Management System | Pet View Supplies</title>
...
<div class="card mb-4 mx-3 mt-4" style="max-height: 700px; overflow-y: auto;">
    <div class="card-body">
        <div class="section-title">
            <h4 class="m-0 text-uppercase font-weight-bold">View Pet Supplies</h4>
        </div>
        <br>
...
<div class="container mb-4">
  <form method="GET" action="">
    <div class="input-group">
      <input type="text" class="form-control" name="search" placeholder="Search by pet supplies name" value="<?php echo htmlspecialchars($searchQuery); ?>">
    ...
    <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
      <div class="col-md-4 mb-4"> <!-- Each card takes 4 columns, making it 3 cards per row -->
        <div class="card" style="width: 100%; height: 29rem;">
          <img class="card-img-top" src="<?php echo htmlspecialchars($row['pet_image']); ?>" alt="Card image cap" style="object-fit: cover; height: 12rem;">
          <div class="card-body d-flex flex-column">
            ...
            <span class="price" style="font-weight: bold; text-align: center;">₱<?php echo number_format($row['pet_price'], 2); ?></span>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="editModalLabel">Edit Pet Product</h5>
      ...
      <div class="modal-body">
        ...
        <form id="editForm">
          ...
          <div class="form-group">
            <label for="editPetName">Pet Product Name</label>
            <input type="text" class="form-control" id="editPetName">
          </div>
          <div class="form-group">
            <label for="editPetDescription">Pet Product  Description</label>
            <textarea class="form-control" id="editPetDescription" rows="4"></textarea>
          </div>
          <div class="form-group">
            <label for="editPetPrice">Pet Product  Price</label>
            <input type="text" class="form-control" id="editPetPrice">
          </div>
          <div class="form-group">
            <label for="editPetImage">Pet Product  Image</label>
            <img id="editPetImagePreview" src="" alt="Product Image" class="img-fluid mb-2">
            <input type="file" class="form-control" id="editPetImage" accept="image/*">
          </div>
        </form>
      </div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('.edit-btn').on('click', function() {
      ...
      $.ajax({
        url: 'get_pet_product.php', // The PHP file that will return the product data
        type: 'GET',
        data: { id: id },
        success: function(response) {
          ...
          $('#editPetDescription').val(data.pet_description);
          $('#editPetPrice').val(data.pet_price);
          $('#editPetImagePreview').attr('src', data.pet_image);
    ...
    $('#saveChanges').on('click', function() {
      ...
      formData.append('pet_description', $('#editPetDescription').val());
      formData.append('pet_price', $('#editPetPrice').val());

      // Check if a new file has been selected
      var fileInput = $('#editPetImage')[0];
      if (fileInput.files.length > 0) {
        formData.append('pet_image', fileInput.files[0]);
      }

      // Send the data via AJAX to update the product
      $.ajax({
        url: 'update_pet_product.php', // The PHP file that will process the update
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
        success: function(response) {
          ...
          if (response === 'success') {
            showToast('Pet supplies updated successfully!', 'success');
            // Redirect after a delay
            setTimeout(function() {
              window.location.href = 'pet_view_products.php'; // Modify this if you need to pass parameters
            }, 3000);
          } else {
...
<script type="text/javascript">
  document.getElementById('editPetImage').addEventListener('change', function(event) {
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.delete-btn').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure you want to delete this pet supplies?',
                text: "",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('delete_pet_product.php?id=' + encodeURIComponent(id))
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                showToast('Pet product deleted successfully.','success');
