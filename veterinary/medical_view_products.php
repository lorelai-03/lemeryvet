...
if ($is_logged_in) {
    ...
    if ($searchQuery) {
        $sql .= " AND medical_name LIKE :searchQuery";
    }
...
<head>
  ...
  <title>Veterinary Clinic Management System | Medical View Products</title>
...
<div class="card mb-4 mx-3 mt-4" style="max-height: 700px; overflow-y: auto;">
    <div class="card-body">
        <div class="section-title">
            <h4 class="m-0 text-uppercase font-weight-bold">View Medical Products</h4>
        </div>
        <br>
<!-- Search Form -->
<div class="container mb-4">
  <form method="GET" action="">
    <div class="input-group">
      <input type="text" class="form-control" name="search" placeholder="Search by medical name" value="<?php echo htmlspecialchars($searchQuery); ?>">
    ...
    <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
      <div class="col-md-4 mb-4"> <!-- Each card takes 4 columns, making it 3 cards per row -->
        <div class="card" style="width: 100%; height: 29rem;">
          <img class="card-img-top" src="<?php echo htmlspecialchars($row['medical_image']); ?>" alt="Card image cap" style="object-fit: cover; height: 12rem;">
          <div class="card-body d-flex flex-column">
            ...
            <span class="price" style="font-weight: bold; text-align: center;">₱<?php echo number_format($row['medical_price'], 2); ?></span>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="editModalLabel">Edit Medical Product</h5>
      ...
      <div class="modal-body">
        ...
        <form id="editForm">
          ...
          <div class="form-group">
            <label for="editMedicalName">Medical Name</label>
            <input type="text" class="form-control" id="editMedicalName">
          </div>
          <div class="form-group">
            <label for="editMedicalDescription">Medical Description</label>
            <textarea class="form-control" id="editMedicalDescription" rows="4"></textarea>
          </div>
          <div class="form-group">
            <label for="editMedicalPrice">Medical Price</label>
            <input type="text" class="form-control" id="editMedicalPrice">
          </div>
          <div class="form-group">
            <label for="editMedicalImage">Medical Image</label>
            <img id="editMedicalImagePreview" src="" alt="Product Image" class="img-fluid mb-2">
            <input type="file" class="form-control" id="editMedicalImage" accept="image/*">
          </div>
        </form>
      </div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('.edit-btn').on('click', function() {
      ...
      $.ajax({
        url: 'get_product.php', // The PHP file that will return the product data
        type: 'GET',
        data: { id: id },
        success: function(response) {
          ...
          $('#editId').val(data.id);
          $('#editMedicalName').val(data.medical_name);
          $('#editMedicalDescription').val(data.medical_description);
          $('#editMedicalPrice').val(data.medical_price);
          $('#editMedicalImagePreview').attr('src', data.medical_image);
    ...
    $('#saveChanges').on('click', function() {
      ...
      formData.append('medical_name', $('#editMedicalName').val());
      formData.append('medical_description', $('#editMedicalDescription').val());
      formData.append('medical_price', $('#editMedicalPrice').val());

      // Check if a new file has been selected
      var fileInput = $('#editMedicalImage')[0];
      ...
      $.ajax({
        url: 'update_product.php', // The PHP file that will process the update
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
        success: function(response) {
          ...
          if (response === 'success') {
            showToast('Medical product updated successfully!','success');
...
<script type="text/javascript">
  document.getElementById('editMedicalImage').addEventListener('change', function(event) {
  var file = event.target.files[0];
  if (file) {
    var reader = new FileReader();
    reader.onload = function(e) {
      // Set the preview image src attribute to the file data
      document.getElementById('editMedicalImagePreview').src = e.target.result;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.delete-btn').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure you want to delete this medical product?',
                text: "",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('delete_product.php?id=' + encodeURIComponent(id))
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                showToast('Medical product deleted successfully.','success');
                    // Delay redirection by 3 seconds
                    setTimeout(function() {
                        window.location.href = 'medical_list_products.php'; // Modify this if you need to pass parameters
                    }, 3000);
                            } else {
