...
<head>
  ...
  <title>Veterinary Clinic Management System | Pet List Supplies</title>
...
<div class="card mb-4 mx-3 mt-4">
    <div class="card-body">
        <div class="section-title">
            <h4 class="m-0 text-uppercase font-weight-bold">List Pet Supplies</h4>
        </div>
        <br>
<form id="petProductForm" enctype="multipart/form-data">
    <div class="form-group">
        <label for="pet_name">Pet Supplies Name</label>
        <input type="text" class="form-control" id="pet_name" name="pet_name" placeholder="Enter pet product name" value="">
    </div>
    <div class="form-group">
        <label for="pet_description">Pet Supplies Description</label>
        <textarea class="form-control" id="pet_description" name="pet_description" placeholder="Enter pet product description" rows="3"></textarea>
    </div>
    <div class="form-group">
        <label for="pet_price">Pet Supplies Price</label>
        <input type="text" class="form-control" id="pet_price" name="pet_price" placeholder="Enter pet product price" value="">
    </div>
    <div class="form-group">
<div class="form-group position-relative">
    <img id="pet-img" src="https://via.placeholder.com/400x300.png?text=Pet+Product+Image
" alt="Pet Picture" class="img-fluid">
    <input type="file" name="pet_image" id="pet-upload" accept="image/*" style="display: none;">
    <label for="pet-upload" class="btn btn-primary position-absolute bottom-0 start-50 translate-middle-x">Choose Image</label>
</div>
...
<script>
    $(document).ready(function() {
        $('#petProductForm').on('submit', function(e) {
            ...
            $.ajax({
                url: 'save_pet_product.php', // The URL of your PHP file to handle the form data
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
                    ...
                    try {
                        ...
                        if (res.status === 'success') {
                            // Show success toast
                            showToast('Pet product saved successfully!', 'success');
        ...
        $('#pet-upload').on('change', function() {
            var reader = new FileReader();
            reader.onload = function(e) {
                $('#pet-img').attr('src', e.target.result);
            }
